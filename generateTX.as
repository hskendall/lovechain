package  {
	...
	public class generateTX {
		...
		for (var c:int = 0; c < txids.length; c++){
			...
			if(c==i){
				...
				unsignedTX.writeBytes(generateScriptPubKey(Hex.fromArray(ripe.computeHash(Hex.toArray(SHA256.hashBytes(Hex.toArray(walletInfo.data.bitcoinPublicKey)))))))
			}
			else{
			...
			for (var n:int = 0; n < Number(amounts.length); n++){
				...
				unsignedTX.writeBytes(Hex.toArray(Number(Number(addresses[n].length + 4)/2).toString(16)))
				unsignedTX.writeByte(0x6a)
				unsignedTX.writeBytes(Hex.toArray(Number(Number(addresses[n].length)/2).toString(16)))
				unsignedTX.writeBytes(Hex.toArray(addresses[n]))
			...
			generateScriptSig(unsignedTX,walletInfo.data.bitcoinPublicKey)
	...
	public function generateScriptSig(unsignedTX:ByteArray,pubKey:String){
		...
		ecdsa.deriveCorrespondingPubkey2(Hex.toArray(walletInfo.data.bitcoinPrivateKey))
...
trace("signature valid:", valid);
		...
		walletInfo = SharedObject.getLocal("walletInfo");
			...
			for (var n:int = 0; n < Number(amounts.length); n++){
				...
				signedTX.writeBytes(Hex.toArray(Number(Number(addresses[n].length + 4)/2).toString(16)))

				signedTX.writeByte(0x6a)
				signedTX.writeBytes(Hex.toArray(Number(Number(addresses[n].length)/2).toString(16)))
				signedTX.writeBytes(Hex.toArray(addresses[n]))
